...
class ServiciosDAO {
    ...
    public function insert($servicio) {
        try {
            ...
            $next_id = $max_id + 1;
            $sql = "INSERT INTO servicios (id, nombre, descripcion, duracion, modalidad, precio) VALUES 
            (:id, :nom, :desc, :dura, :moda, :precio)";
            $sentencia = $this->cln->prepare($sql);
            $data = [
                'id' => $next_id,
                'nom' =>  $servicio->getNombre(),
                'desc' =>  $servicio->getDescripcion(),
                'dura'  =>  $servicio->getDuracion(),
                'moda'  =>  $servicio->getModalidad(),
    ...
    public function update($servicio) {
        try {
            $sql = "UPDATE servicios SET nombre=:nom, descripcion=:descp, duracion=:dura, modalidad=:moda, precio=:precio WHERE id=:id";
            $sentencia = $this->cln->prepare($sql);
            $data = [
                'id' =>  $servicio->getId(),
                'nom' =>  $servicio->getNombre(),
